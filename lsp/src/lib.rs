...
use lsp_types::{
    ...
    InitializeParams, LanguageString, MarkedString, ServerCapabilities, TextDocumentPositionParams,
...
pub fn kb_hover(text_params: TextDocumentPositionParams) -> Option<KbCompletion> {
    ...
    match result {
        ...
        Position::AttributeValue { name, .. } => KB_TAGS.iter().find(|x| x.name == name).cloned(),
    }
}
