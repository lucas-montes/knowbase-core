...
bool Database::pathExists(const std::string &filepath)
{
    ...
    if (sqlite3_bind_text(stmt, 1, filepath.c_str(), filepath.length(), SQLITE_STATIC) != SQLITE_OK)
    {
...
{
    const char *createTableSQL = "CREATE TABLE IF NOT EXISTS files (id INTEGER PRIMARY KEY, filepath TEXT);";
    char *errMsg = nullptr;
...
bool Database::insertData(const std::string &data)
{
...
bool Database::deleteData(const std::string &filepath)
{
    ...
    sqlite3_bind_text(stmt, 1, filepath.c_str(), filepath.length(), SQLITE_STATIC);
