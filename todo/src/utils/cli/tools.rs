...
impl RelateArgs {
    async fn run(&self) -> i16 {
        match &self.from{
            Models::Projects => GoalsFile::handle_relationships(self.id, self.to, &self.action).await,
            Models::Tasks => ProjectsFile::handle_relationships(self.id, self.to, &self.action).await,
    }
    }
}
