...
use crate::projects::ProjectsFile;
use crate::tasks::{Task, TasksFile};
use crate::utils::FileSaver;
...
async fn get_all_to_do_tasks(actives_tasks: &Vec<Task>, now: NaiveDateTime) -> Vec<&Task> {
...
async fn get_all_active_tasks(all_tasks: &mut std::collections::HashMap<i16, Task>) -> Vec<Task> {
